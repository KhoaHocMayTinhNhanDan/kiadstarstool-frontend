...
import type { IconProps, IconWithStatusProps } from './Icon.types';
import { 
  ICON_CONFIG,
  ICON_I18N_KEYS 
} from './Icon.constants';
...
export const Icon = forwardRef<HTMLDivElement, IconProps>(
  (
    {
      // Core props
      variant = ICON_CONFIG.defaults.variant,
      size = ICON_CONFIG.defaults.size,
      children,
      label,
    ...
    const variantStyle = ICON_CONFIG.getVariantStyle(variant);
    const sizeStyle = ICON_CONFIG.getSizeStyle(size);
    
    // Determine content
    const content = children || label || t(ICON_I18N_KEYS.defaultLabel, 'Icon');
    
    // Determine aria-label
    const resolvedAriaLabel = ariaLabel || 
      t(ICON_I18N_KEYS.ariaLabel, 'Icon');
...
export const IconWithStatus = forwardRef<HTMLDivElement, IconWithStatusProps>(
...
export const IconGroup: React.FC<{
  ...
  return (
    <div style={{ 
      ...
      {remaining > 0 && (
        <div style={{
          width: ICON_CONFIG.getSizeStyle(size).size,
          height: ICON_CONFIG.getSizeStyle(size).size,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          backgroundColor: '#e5e7eb',
          borderRadius: '50%',
          fontSize: ICON_CONFIG.getSizeStyle(size).fontSize * 0.8,
          color: '#6b7280',
          fontWeight: 600,
        }}>
          +{remaining}
