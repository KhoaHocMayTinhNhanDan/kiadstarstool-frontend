// src/04-frameworks-and-drivers/ui/web/components/atoms/Logo/Logo-test.tsx
import { useState } from 'react';
import { Logo, LogoWithStatus, LogoGroup } from './Logo';
import { useI18n } from '@/shared/i18n';
import { LOGO_CONFIG } from './Logo.constants';
...
export function LogoTest() {
  ...
  const handleClick = () => {
    ...
    alert(t(LOGO_I18N_KEYS.clickMessage, 'Logo clicked!'));
  };

  const variants = LOGO_CONFIG.variants;
  const sizes = LOGO_CONFIG.sizes;

  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      <h2 style={{ marginBottom: 24, color: '#333' }}>🧪 Logo Component Tests</h2>
          ...
          {sizes.map(size => (
            <div key={size} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: '#666', marginTop: 4 }}>
                {LOGO_CONFIG.getSizeStyle(size).size}px
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <Logo variant="primary" disabled>
            Disabled
          </Logo>
          <Logo variant="primary" loading>
            {t(LOGO_I18N_KEYS.loadingText, 'Loading...')}
          </Logo>
          <Logo variant="primary" selected>
            Selected
          </Logo>
          <Logo 
            variant="primary" 
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <LogoGroup spacing={12}>
            <Logo variant="primary">Item 1</Logo>
            <Logo variant="secondary">Item 2</Logo>
            <Logo variant="default">Item 3</Logo>
            <Logo variant="primary">Item 4</Logo>
            <Logo variant="secondary">Item 5</Logo>
          </LogoGroup>
        </div>
        <div>
          <h4 style={{ marginBottom: 8, color: '#666' }}>With Max Limit</h4>
          <LogoGroup max={3} spacing={8}>
            <Logo size="sm" variant="primary">A</Logo>
            <Logo size="sm" variant="secondary">B</Logo>
            <Logo size="sm" variant="default">C</Logo>
            <Logo size="sm" variant="primary">D</Logo>
            <Logo size="sm" variant="secondary">E</Logo>
          </LogoGroup>
      ...
      <section style={{ 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 16 }}>
          ...
          <div>
            ...
            <ul style={{ margin: '8px 0 0 0', paddingLeft: 20 }}>
              {sizes.map(s => (
                <li key={s}>{s}: {LOGO_CONFIG.getSizeStyle(s).size}px</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul style={{ margin: '8px 0 0 0', paddingLeft: 20 }}>
              <li>Variant: {LOGO_CONFIG.defaults.variant}</li>
              <li>Size: {LOGO_CONFIG.defaults.size}</li>
            </ul>
          </div>
        </div>
      </section>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 400 }}>
          ...
          <div style={{ padding: 12, backgroundColor: '#e9ecef', borderRadius: 6 }}>
            ...
            <div style={{ marginTop: 4, fontSize: 14 }}>
              ...
              Size: <code>{selectedSize}</code> ({LOGO_CONFIG.getSizeStyle(selectedSize).size}px),
